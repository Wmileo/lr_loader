...
if [[ $b_method == "production" ]] && [[ $b_env == "prod" ]]; then
  yarn cross-env LR_TYPE=package VITE_ENV="${b_env}" LR_COVER=1 uni build -p mp-weixin-prod
else 
  yarn cross-env LR_TYPE=package VITE_ENV="${b_env}" LR_COVER=1 uni build -p mp-weixin
fi
